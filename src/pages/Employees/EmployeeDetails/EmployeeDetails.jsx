          ...
          <div className="flex -items-center">
            ...
            <div className="flex items-center gap-1">
              <div className="w-6 h-6 rounded-full overflow-hidden">
                <img src={employeeData?.Image} alt="user" className="w-full h-full object-contain"/>
              </div>
              {employeeData?.Employee}
            </div>
          </div>
      ...
      <div className="grid md:grid-cols-2 grid-cols-1 gap-3 mt-4">
        <InfoCard title={"Date"}>
          <div className="flex gap-6 items-center px-3 py-5">
            <div className="font-medium text-[var(--secondary-color)]">start date</div>
            <p>{new Date(employeeData?.StartDate).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' }).replace(/-/g, ' / ')}</p>
          </div>
        </InfoCard>
